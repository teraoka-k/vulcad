...
public:
  ...
  CommandBuffer(VkCommandPool commandPool, VkDevice device, VkExtent2D extent,
    ...
    this->vertexBuffer = VertexBuffer(device, physicalDevice, vertices,
                                      commandPool, graphicsQueue);
    this->indexBuffer = IndexBuffer(device, physicalDevice, indices,
                                    commandPool, graphicsQueue);
...
#endif // VULCAD_CORE_VULKAN_COMMAND_COMMAND_BUFFER
